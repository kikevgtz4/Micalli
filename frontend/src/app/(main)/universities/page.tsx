...
export default function UniversitiesPage() {
  ...
  return (
    <MainLayout>
      <div className="bg-stone-50 py-10 min-h-screen pt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {universities.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-stone-600">No universities found. Please add universities through the admin interface.</p>
              <a 
                href="http://localhost:8000/admin/universities/university/add/" 
                target="_blank"
                rel="noopener noreferrer" 
                className="inline-block mt-4 px-4 py-2 bg-primary-500 text-white rounded hover:bg-primary-600"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {universities.map((university) => (
                <div key={university.id} className="bg-surface rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 ">
                  <div className="h-40 bg-primary-100 flex items-center justify-center">
                    {university.logo ? (
                      ...
                      <span className="text-primary-700 font-semibold text-xl">{university.name.charAt(0)}</span>
                    )}
                  </div>
                  <div className="p-6">
                    <h2 className="text-xl font-semibold text-stone-900">{university.name}</h2>
                    <p className="text-stone-500 text-sm mt-1">{university.address}</p>
